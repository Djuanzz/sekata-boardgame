...
# --- Global Game State (In-memory, untuk kesederhanaan) ---
...
try:
    ...
    # Tambahkan beberapa kata utuh dari POTONGAN_KATA untuk demo
...
class SeKataHTTPHandler(SimpleHTTPRequestHandler):
    ...
    def handle_join_game(self, path):
        try:
            ...
            if game.add_player(player_id): # Menggunakan metode add_player dari kelas Game
    ...
    def handle_submit_fragment(self, path):
        try:
            ...
            position = request_data.get('position') # 'before' or 'after'
            ...
            if not is_valid:
                player.add_cards([submitted_fragment.upper()]) # Kembalikan kartu ke tangan
                self.send_json_response(400, {"success": False, "message": msg})
                return
            
            # --- Jika Valid, Update Game State ---
            # Masukkan kartu lama di meja ke discard pile
            ...
            player.score += score_earned # Tetap update skor, walau menang dari kartu habis
            
            # Cek pemenang setelah kartu digunakan
            ...
            game.next_turn(action_was_check=False) # Lanjut ke giliran berikutnya

            print(f"Pemain {player_id} menyambung '{submitted_fragment}' ke '{game.card_on_table}'. Kata terbentuk: '{formed_word}'.")
            ...
            # Pastikan kartu dikembalikan jika ada error tak terduga setelah remove_card
            # Ini butuh logika rollback yang lebih canggih atau wrapping transaksi.
    ...
    def handle_check_turn(self, path):
        try:
            ...
            game.next_turn(action_was_check=True) # Beri tahu bahwa aksi adalah 'Check'

            # Cek apakah setelah next_turn (yang mungkin melakukan reshuffle) ada pemenang
            if game.winner:
...
# --- Main Server Setup ---
def run_server(server_class=HTTPServer, handler_class=SeKataHTTPHandler, port=8000):
    server_address = ('', port)
    httpd = server_class(server_address, handler_class)
    print(f"Memulai HTTP server SeKata di http://localhost:{port}/")
